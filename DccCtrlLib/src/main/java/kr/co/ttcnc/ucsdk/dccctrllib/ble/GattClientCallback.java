...
public class GattClientCallback extends BluetoothGattCallback {
    ...
    public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
        ...
        if (status == BluetoothGatt.GATT_SUCCESS) {
            ...
            mClientActionListener.cryptoTokenProcessEnd();
        } else {
    ...
    public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
        if (status == BluetoothGatt.GATT_SUCCESS) {
            ...
            mClientActionListener.cryptoTokenProcessStart();
        } else {
    ...
    private void enableCharacteristicNotification(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
        @SuppressLint("MissingPermission") boolean characteristicWriteSuccess = gatt.setCharacteristicNotification(characteristic, true);
        if (characteristicWriteSuccess) {
            mClientActionListener.log("Characteristic notification set successfully for " + characteristic.getUuid().toString());
            if (BluetoothUtils.isWriteCharacteristic(characteristic)) {
                mClientActionListener.log("Characteristic initializeWrite");
            } else if (BluetoothUtils.isReadCharacteristic(characteristic)) {
    ...
    private void enableCharacteristicConfigurationDescriptor(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
        ...
        @SuppressLint("MissingPermission") boolean descriptorWriteInitiated = gatt.writeDescriptor(descriptor);
    ...
    private void readCharacteristic(BluetoothGattCharacteristic characteristic) {
        ...
        mClientActionListener.log("Read: " + Utils.byteArrayInHexFormat(messageBytes));
